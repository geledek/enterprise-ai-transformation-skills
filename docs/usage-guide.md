...
|---|---|---|---|
| 1 | `maturity-assessment` | Quarterly or before any AI strategy refresh — where are we on the maturity curve? | Foundational baseline |
| 2 | `use-case-discovery` | "Where do we even start with AI?" — generate and rank candidates | Front of funnel |
| 3 | `ai-idea-diagnostic` | Gate any individual AI concept before pilot or build | Concept gate |
| 4 | `data-trust-deployment-pattern` | "Where can this data legally and safely run?" | Deployment-tier mapping |
| 5 | `pilot-design` | Structure a 90-day pilot with falsifiable kill criteria | Pilot rigor |
| 6 | `roi-gate-pwc` | Funding gate — approve, kill, send back, fund-with-condition | Investment gate |
| 7 | `buy-vs-build` | Sourcing decision — buy / build / partner | Sourcing |
| 8 | `productionization-playbook` | Move from working prototype to SLO-bound production | Pilot-to-prod |
| 9 | `agent-guardrail-imda` | Govern a deployed agent against IMDA's 4-dimension framework | Deployment governance |
| 10 | `stack-diagnostic` | "Is our tech stack ready to scale?" | Tech foundation |
| 11 | `ai-portfolio-observability` | Run AI as a portfolio, not as scattered pilots | Run-state visibility |
| 12 | `readiness-conversation` | Surface where leadership / operator / HR / board gaps will block progress | Readiness diagnostic |
| 13 | `workforce-literacy-curriculum` | Stand up role-based literacy + EU AI Act Art. 4 evidence | People |
| 14 | `augmentation-frontline-engagement` | Engage skeptical frontline experts as co-designers, protect tradecraft | People |
| 15 | `peer-case-library` | "What have others actually done?" — pull closest-match analogs | Reference |
...
### Example 1: `ai-idea-diagnostic` — Restaurant chain founder considers an AI chatbot
...
Claude invokes `ai-idea-diagnostic` because the prompt matches trigger phrase "we should build AI for X — is it a good idea?" (concept-stage, pre-pilot, pre-investment).
...
Priya pulled the RFP. The exec team redirected SGD 280K of the original envelope to a kitchen-prep demand-forecast pilot (the remaining SGD 70K went to fixing the loyalty-to-POS join and adding allergen/halal fields, which become prerequisites for any future customer-facing AI). Priya then ran `pilot-design` on the prep-forecast use case to set baseline metrics and a 90-day evaluation gate.
...
### Example 2: `data-trust-deployment-pattern` — Clinical-notes summarization in a Singapore hospital
...
Claude invokes `data-trust-deployment-pattern` because the prompt matches the trigger phrases "what deployment pattern fits regulated data?" and "consumer-tier vs enterprise-tier tool — which one applies?" plus the explicit shadow-AI suppression ask.
...
Annie took the verdict to Friday's steering meeting and got sign-off to fund the Azure OpenAI SG deployment plus the Presidio redaction pipeline. The CMIO disclosed his consumer-ChatGPT use under the amnesty; the DPO's PDPA breach analysis came in below the notifiable threshold and was filed. Annie then chained `pilot-design` to scope the 12-physician pilot and `productionization-playbook` to define the eval harness and the <1.5% factual / <0.1% safety-critical thresholds.
...
### Example 3: `productionization-playbook` — Solo founder taking a PropTech prototype to production
...
Claude invokes `productionization-playbook` because the prompt matches trigger phrases "demo works but production breaks", "need SLOs", "staged rollout plan", and "go-live readiness gate".
...
Wei messaged Marcus that night, sent the renegotiated phased-launch SOW Wednesday morning, Astoria signed Thursday with the carved-out business-hours SLA and S$42k repricing. She then ran the `roi-gate-pwc` skill against the new 14-week ramp to recheck whether the unit economics still cleared the 200% MROI threshold post-repricing.
...
### Example 4: `agent-guardrail-imda` — Governing a deployed credit-decisioning agent at a regional bank
...
Claude invokes `agent-guardrail-imda` because the prompt matches trigger phrase "review guardrails for this AI agent" and explicitly references agent governance with a regulator (MAS) in scope.
...
**Skill chain:** `maturity-assessment` → `use-case-discovery` → `ai-idea-diagnostic` → `data-trust-deployment-pattern` → `pilot-design` → `roi-gate-pwc` → `productionization-playbook` → `ai-portfolio-observability`
...
**Run** — `maturity-assessment` runs the bank against the MIT CISR 4-stage ladder (Experimenter → Player → Transformer → Pacesetter) cross-referenced with Accenture's AI Maturity Index (Foundation / Experimenter / Innovator / Achiever) along five dimensions. r17 convenes a 90-minute working session: herself, the Group CDO, the Head of Risk Tech, the COO's chief of staff, and the Head of Talent. Skill walks them through scored evidence, not opinion.
...
**What this hands to the next skill (`use-case-discovery`):**
...
- **Capacity ceiling**: realistic Y1 throughput is 2-3 lighthouse pilots + 1 platform programme, given 14 AI staff, 14-week central queue, no MLEs FTE. `use-case-discovery` must hard-cap shortlist accordingly.
...
- **Strategic posture**: position Y1 as "earn the right to scale" — 2-3 wins with attributable P&L, a platform that ends shadow AI, and a governance register that survives MAS thematic review; this becomes the framing brief that `use-case-discovery` opens with.

### Skill 2: `use-case-discovery`
...
**What this hands to the next skill (`ai-idea-diagnostic`):**
...
Verdict: data is workable but not free. ~10-12 weeks of unglamorous work before any model touches a memo. This is the Stage 2 maturity tax — Skill 1 already flagged the weak data foundation. Skill 4 (`data-trust-deployment-pattern`) will inherit this.
...
**What this hands to the next skill (`data-trust-deployment-pattern`):**
...
### Skill 4: `data-trust-deployment-pattern`
...
**What this hands to the next skill (`pilot-design`):**
...
- **Open items deferred to `pilot-design`:** banker UX for showing PII re-hydration without re-introducing leak risk; sample size and stratification (must include EU-UBO cases); shadow-mode duration before banker-visible mode; kill-switch ownership.

### Skill 5: `pilot-design`
...
**What this hands to the next skill (`roi-gate-pwc`):**
...
**What this hands to the next skill (`productionization-playbook`):**
...
### Skill 7: `productionization-playbook`
...
**What this hands to the next skill (`ai-portfolio-observability`):**
...
### Skill 8: `ai-portfolio-observability`
...
Quarterly: re-run AI maturity-assessment (Skill 1) to confirm stage progression and rebalance the portfolio. Retire underperformers, double down on greens.
...
- Next quarterly cycle: re-run Skill 1 (maturity-assessment) to confirm stage progression from "scaling" to "transforming," rebalance the portfolio (retire KYC v1, scope agent #5), and refresh the 18-month roadmap with the four agents now treated as run-rate, not projects
...
**Skill chain:** `readiness-conversation` → `workforce-literacy-curriculum` → `augmentation-frontline-engagement`

### Skill 1: `readiness-conversation`
...
**What this hands to the next skill (`workforce-literacy-curriculum`):**
...
### Skill 2: `workforce-literacy-curriculum`
...
**What this hands to the next skill (`augmentation-frontline-engagement`):**
...
### Skill 3: `augmentation-frontline-engagement`
...
Claude invokes `agent-guardrail-imda` because the prompt matches the trigger phrase "agentic governance check" and "is our agentic AI safe to deploy" semantics.
...
### Step 2 — `ai-portfolio-observability`
...
Claude invokes `ai-portfolio-observability` because the prompt matches "deployed agents but can't see them," "unclear ROI," "token spend exploding," and "what's the net ROI after oversight."
...
`agent-guardrail-imda` did the work no portfolio dashboard can do — it forced naming an accountable individual, scoping tools structurally rather than via prompt, and confronting the rubber-stamp pattern in RM sign-off. Without it, the observability layer would have prettier numbers on top of the same governance vacuum.

`ai-portfolio-observability` did the work no per-agent governance check can do — it subtracted oversight cost across the portfolio, exposed the −38% gap between gross and net, ranked agents into a sunset queue, and tied tokens to a BU P&L line so spend is no longer no-one's problem.
...
Claude invokes `ai-idea-diagnostic` because the prompt matches the trigger phrases "should we pursue this AI idea" and "is this AI concept worth exploring" applied to three concept-stage ideas before any pilot or build commitment.

---

### Step 1 — `ai-idea-diagnostic` (run three times, one per candidate idea)
...
Claude invokes `buy-vs-build` because the prompt matches the trigger phrases "which vendor should we use for AI" and "help me decide on the AI sourcing strategy" — and applies the NANDA 2:1 buy advantage given the no-engineering constraint.
...
Claude invokes `stack-diagnostic` because the prompt matches the trigger phrase "diagnose our AI tech stack" applied to the proposed sanctioned bundle, and asks specifically about shadow-AI risk (Layer 4).
...
`ai-idea-diagnostic` did the killing. Without it, Priya would have spent the budget on lead scoring (Idea 2) — the worst of the three — because it was the most concrete pitch. The Reframe verdict on Idea 2 alone paid for the session.

`buy-vs-build` did the constraining. Once Idea 1 and Idea 3 survived, the NANDA 2:1 frame and the SME-scale component map killed three temptations — fine-tuning, paying for Copilot in parallel, and hiring a systems integrator. It also named the moat (the two prompt libraries) so Priya's build effort lands where it compounds.

`stack-diagnostic` did the surfacing. The technical stack passed easily — but the behavioural layer was already failing in the form of shadow AI on healthcare client data. Without the diagnostic, Priya would have sanctioned a clean bundle while the unsanctioned one continued to leak data underneath it.
...
**Skill chain:** `readiness-conversation` → `augmentation-frontline-engagement` → `workforce-literacy-curriculum` → `agent-guardrail-imda`
...
**What this hands to next skill (`augmentation-frontline-engagement`):**
...
### Skill 2: `augmentation-frontline-engagement`
...
**What this hands to next skill (`workforce-literacy-curriculum`):**
...
### Skill 3: `workforce-literacy-curriculum`
...
**What this hands to next skill (`agent-guardrail-imda`):**
...
2. **Diagnosis vs synthesis.** The diagnostic skills (`ai-idea-diagnostic`, `agent-guardrail-imda`, `roi-gate-pwc`) compare the new situation against the closest case and name the parallel explicitly in the synthesis step. This keeps verdicts grounded in priors instead of generic principles.
...
3. **Fork a skill.** Copy any `SKILL.md`, rename it (e.g. `ai-idea-diagnostic-financial-services`), adjust the role instructions and reference pointers for your sector, internal processes, or role-specific context. The forked skill lives next to the original; the agent loads whichever matches the trigger phrasing best.
